...
class WebServer {
	...
	createHTTPSRedirect () {
		let _this = this;
		_this.Config.httpServer = HTTP.createServer((req, res) => {
			res.writeHead(
				301,
				{
					"Location": "https://" + req.headers['host'].replace(/:[0-9]+/, '') + (_this.Config.httpsPort !== 443 ? ':' + _this.Config.httpsPort : '') + req.url
				}
			);
			res.end();
